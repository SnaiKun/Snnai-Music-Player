...
async fn search_soundcloud_ytdlp(app: tauri::AppHandle, query: String, limit: u32) -> Result<Vec<Track>, String> {
    ...
    for line in stdout.lines() {
        ...
        if artwork_url.is_empty() {
            artwork_url = "data:image/svg+xml,<svg xmlns=\"http://www.w3.org/2000/svg\" viewBox=\"0 0 100 100\"><rect width=\"100\" height=\"100\" fill=\"%23ff5500\"/><path d=\"M20,60 C20,50 30,4[...]
        }
            ...
            if resp.status().is_success() {
                ...
                for item in collection {
                    ...
                    if !artwork_url.is_empty() {
                        ...
                        artwork_url = "data:image/svg+xml,<svg xmlns=\"http://www.w3.org/2000/svg\" viewBox=\"0 0 100 100\"><rect width=\"100\" height=\"100\" fill=\"%23ff5500\"/><path d=\"M20,60[...]
                    }
...
/// Fetch audio bytes with retry logic and exponential backoff.
/// Increases timeout to 120s to accommodate large audio files and slow connections.
async fn fetch_audio_bytes_internal(url: &str) -> Result<Vec<u8>, String> {
    let client = reqwest::Client::builder()
...
async fn fetch_audio_bytes(url: String) -> Result<Vec<u8>, String> {
    ...
    for attempt in 1..=MAX_RETRIES {
        match fetch_audio_bytes_internal(&url).await {
            Ok(bytes) => {
                eprintln!("[DEBUG] fetch_audio_bytes succeeded on attempt {}", attempt);
                return Ok(bytes);
            }
            Err(e) if attempt < MAX_RETRIES => {
                let backoff_secs = 2u64.pow(attempt - 1);
                eprintln!("[RETRY] Attempt {} failed: {}. Retrying in {} seconds...", attempt, e, backoff_secs);
                tokio::time::sleep(tokio::time::Duration::from_secs(backoff_secs)).await;
            }
            Err(e) => {
                eprintln!("[FAILED] All {} retry attempts exhausted: {}", MAX_RETRIES, e);
                return Err(format!("Failed to fetch audio after {} attempts: {}", MAX_RETRIES, e));
            }
        }
    }
    
    Err("Unexpected error: retry loop exited unexpectedly".to_string())
}
